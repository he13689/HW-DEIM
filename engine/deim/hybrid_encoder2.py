...
class EnCover(nn.Module):
    ...
    def forward(self, x, src_mask=None, pos_embed=None):
        residual = x
...
@register()
class GHybridEncoder(nn.Module):
    __share__ = ['eval_spatial_size', ]
    ...
    def __init__(self,
        ...
        self.foc = False
        if self.foc:
            self.foc_m = Foc(in_channels[1:], hidden_dim, hidden_dim, r=2.)
        ...
        self.hyper_graph = HyperGraph(hidden_dim, hidden_dim)

        self.downsample = nn.Conv2d(hidden_dim, hidden_dim, 3, stride=2, padding=1)
        self.fuse_conv = ConvNormLayer_fuse(hidden_dim * 2, hidden_dim, 1, stride=1, padding=0, act='silu')
        # self.upsample = nn.ConvTranspose2d(hidden_dim, hidden_dim, 3, stride=2, padding=1, output_padding=1)
    ...
    def hgfuse(self, x1, x2):
        # y1, y2 = torch.split(x1, 256, 1)
        y1 = self.downsample(x1)
        y1 = self.fuse_conv(torch.cat([y1, x2], dim=1))
        return y1

    def forward(self, feats):
        assert len(feats) == len(self.in_channels)
        if self.foc:
            feats[1:] = self.foc_m(feats[1:])
        proj_feats = [self.input_proj[i](feat) for i, feat in enumerate(feats)]

        # 加入hypervce [256 80 80， 256 40 40， 256 20 20] -> 256 40 40
        hg = self.hyper_graph(proj_feats)
        # outputs/deim_dfine_algae_m原始网络结果：AP5095: 0.4870251074419194，params: 19551537
        # 只在40 40上加入超图结果， AP5095: 0.46828311189763094, params: 20829745
        # hdeim_dfine_hgnetv2_algae_m在 40 40和 20 20 上都加入超图结果 AP5095: 0.4592164786366325 params: 24434225
        # hdeim_dfine_hgnetv2_algae_m2在 20 20 上加入超图结果 59.3712 GFLOPS, AP5095: 0.4773829977294003, params: 21419825
        # hdeim_dfine_hgnetv2_algae_m3在 20 20 上加入超图结果, 并且去掉下采样流程，将超图视为一个全局融合信息 52.5355 GFLOPS AP5095: 0.4786454447626858 params: 18072843
        # hdeim_dfine_hgnetv2_algae_m4在 20 20 上加入超图结果, 去掉下采样，修改超图融合 52.6406 GFLOPS AP5095: 0.4814190919410086 params: 18388785
        # hdeim_dfine_hgnetv2_algae_m5在 20 20 上加入超图结果, 去掉下采样，修改超图融合, 不加fuse_conv的silu 52.6405 GFLOPS GFLOPS AP5095: 0.4539317893990698  params: 18204171
        # hdeim_dfine_hgnetv2_algae_m6在 20 20 上加入超图结果, 去掉下采样，修改超图融合, 加入Encover注意力 number of trainable parameters: 19052449 53.188 GFLOPS AP5095: 0.4863234068366555 params: 18867835
        # hdeim_dfine_hgnetv2_algae_m7, m6上加入汉伟改进，91.3129 GFLOPS params: 38266491 AP5095: 0.4669591868858894
        proj_feats[2] = self.hgfuse(hg, proj_feats[2])

        # hg = self.defuse(hg)
        # hgs, hgm, hgl = self.defuse(hg)
        # proj_feats[2] += hgs
        # proj_feats[1] += hgm
        # proj_feats[0] += hgl
        ...
        # 超图加到后面试一下  0.4670429553797935
        # hg = self.hyper_graph(inner_outs)
        # inner_outs[2] += self.hgfuse(hg)
